...
## 7) Quy trình Git để tránh trùng và conflict

## 7.1 Branching
- `main`: ổn định, chỉ merge qua PR.
- `dev`: tích hợp hằng ngày.
- Feature branch theo task:
  - `feat/BE-01-chat-endpoint`
  - `feat/RAG-01-indexing`
  - `feat/FE-01-chat-ui`
  - `chore/OPS-01-ci-logging`

## 7.2 Quy tắc commit
- Conventional commits:
  - `feat(api): add chat endpoint with citations`
  - `feat(rag): add chunking and retrieval pipeline`
  - `feat(web): render citations and feedback buttons`
  - `chore(ci): add lint and test workflow`
- Mỗi commit chỉ 1 mục tiêu rõ ràng.

## 7.3 PR checklist bắt buộc
- [ ] Không sửa file ngoài ownership nếu chưa thống nhất.
- [ ] Rebase/sync `dev` trước khi mở PR.
- [ ] CI pass.
- [ ] Có test hoặc ảnh demo cho thay đổi UI/API.
- [ ] Có reviewer đúng ownership phụ.

## 7.4 Khung làm việc hằng ngày
1. Daily 10-15 phút: chốt task trong ngày.
2. Cập nhật issue status: `todo -> in progress -> review -> done`.
3. Mỗi tối merge vào `dev` sau khi pass review.
4. Freeze trước demo 1 ngày, chỉ fix bug.
...
- Code chạy local ổn định.
- Có test tối thiểu cho logic chính.
- Có log/trace cơ bản cho debug.
- CI xanh.
- Đã review và merge đúng branch.
- Có cập nhật tài liệu ngắn trong `README`/`docs`.

---

## 9) Rủi ro chính và phương án dự phòng
...
| Conflict code giữa thành viên | Trung bình | Ownership theo folder + PR nhỏ + review chéo cố định |
| Chậm tiến độ | Trung bình | Ưu tiên MVP scope, cắt bớt intent ít dùng |

## 9.1 Risk register mở rộng (khi triển khai thực tế)
...
## 9.2 Mitigation checklist trước khi merge
...
## 9.4 Golden set và regression test
